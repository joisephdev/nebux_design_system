...
class AlphaScroller extends StatefulWidget {
  ...
  final Widget Function(BuildContext, int, String, dynamic) itemBuilder;
  ...
  @override
  State<AlphaScroller> createState() => _AlphaScrollerState();
}

class _AlphaScrollerState extends State<AlphaScroller> {
  void init() {
    widget.list.sort(
    ...
    if (widget.isAlphabetsFiltered) {
      ...
      for (var letter in alphabets) {
        final Country? firstAlphabetElement = _list.firstWhereOrNull(
  ...
  void calculateFirstIndex() {
    for (var letter in _filteredAlphabets) {
      final Country? firstElement = _list.firstWhereOrNull(
        (item) => item.name.toLowerCase().startsWith(letter),
      );
      if (firstElement != null) {
        final int index = _list.indexOf(firstElement);
        ...
        ListView.builder(
          ...
          itemBuilder: (a, x) {
            return ConstrainedBox(
              ...
              child: widget.itemBuilder(a, x, _list[x].name, Country),
            );
          },
        ),
        Align(
            ...
            : ValueListenableBuilder<Offset>(
                ...
                builder:
                    (BuildContext context, Offset position, Widget? child) {
                      return Positioned(
                        right: widget.alignment == LetterAlignment.right
                            ? 40
                            : null,
                        left: widget.alignment == LetterAlignment.left
                            ? 40
                            : null,
                        top: position.dy,
                        child: widget.overlayWidget == null
                            ...
                            : widget.overlayWidget!(
                                _filteredAlphabets[_selectedIndexNotifier
                                    .value],
                              ),
                      );
                    },
